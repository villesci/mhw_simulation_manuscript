...
three_species<-ggplot()+
  ...
  #chronic-acute
  geom_point(aes(x=equ_res1$tempatdeath,y=equ_res1$timetodeath),shape=1)+
  geom_segment(aes(x=25,xend=equ_res1$tempatdeath,y=equ_res1$timetodeath,yend=equ_res1$timetodeath),color="black",linetype="dashed",inherit.aes=F,linewidth=.4)+
  geom_segment(aes(x=equ_res1$tempatdeath,xend=equ_res1$tempatdeath,y=0,yend=equ_res1$timetodeath),color="black",linetype="dashed",inherit.aes=F,linewidth=.4)+
  annotate("text",x=27,y=5000,label=paste0(round((equ_res1$timetodeath/24),digits=1)," Hours, \n ",equ_res1$tempatdeath,"°C "),
           size=4)+
  #chronic-mixed
  geom_point(aes(x=equ_res2$tempatdeath,y=equ_res2$timetodeath),shape=1)+
  ...
  annotate("text",x=41,y=0.2,label=paste0(round((equ_res2$timetodeath/24),digits=1)," Hours, \n ",equ_res2$tempatdeath,"°C "),
           size=4)+
  #acute-mixed
    geom_point(aes(x=equ_res3$tempatdeath,y=equ_res3$timetodeath),shape=1)+
  ...
  annotate("text",x=40,y=9000,label=paste0(round((equ_res3$timetodeath/24),digits=1)," Hours, \n ",round(equ_res3$tempatdeath,digits=1),"°C "),
           size=4)

three_species
ggsave(path = "result_figs/supp",filename="threespecies_TDT.pdf", device='pdf', dpi=300,width=6,height=4,units="in")
```

```{r}

summary(lm(log10(simulated_point$time)~simulated_point$temp*simulated_point$species))


tempi<-unique(simulated_point$temp)

result.df<-data.frame("temp"=tempi,"p-valuea"=NA,"p-valueb"=NA)

for (i in 1:length(tempi)){
  
  tempii<-simulated_point%>%filter(temp==tempi[i])
  
  res<-summary(lm(log10(tempii$time)~tempii$species))
  result.df[i,2]<-coef(res)[2,4]
  result.df[i,3]<-coef(res)[3,4]
...
thick_legend<-get_legend(ggplot(category_prop_species,aes(x=hours_dur,y=magnitude,z=cum.mort,group=species))+geom_tile(aes(fill=cum.mort),size=.5)+
...
species_heatmap_plot<-ggplot(category_prop_species,aes(x=hours_dur,y=magnitude,z=cum.mort,group=species))+geom_tile(aes(fill=cum.mort),size=.5)+
  scale_fill_viridis_c()+
  theme_classic()+
  geom_rect(data=category_prop_species2,aes(xmin=min_duration/60,xmax=max_duration/60,ymin=min_magnitude,ymax=max_magnitude,color=category),fill=NA,inherit.aes=F,linewidth=1)+
  #vertical
  geom_segment(aes(x=8.2*12,xend=8.2*12,y=.75-.075,yend=8.27),color="cornsilk3",lwd=1)+
  #horizontal
   geom_segment(aes(x=8.2*12,xend=61.5*12,y=.75-0.03,yend=.75-0.03),color="cornsilk3",lwd=1)+
  scale_color_manual(values=MHW_colours)+
  geom_contour(color="white")+ geom_text_contour(aes(z = cum.mort),color="white",fontface="bold")+
  labs(color="MHW Category", fill="Survival (%)")+
  xlab("Duration (hours)")+facet_wrap(.~label)+
...
ggsave(path = "result_figs",filename="species_heatmap_three.pdf", device='pdf', dpi=300,width=8,height=4,units="in")
```
...
species_arranged_plot_legend<-plot_grid(species_arranged_plot,thick_legend,ncol=2,rel_widths=c(1,0.2))
ggdraw(species_arranged_plot_legend+theme(plot.background = element_rect(fill = "white", color = NA)
))

ggsave(path = "result_figs",filename="species_combinedplots_three.pdf", device='pdf', dpi=300,width=8,height=8,units="in")
...
alphas_arranged_plot_legend<-plot_grid(annual_arranged_plot,thick_legend,ncol=2,rel_widths=c(1,0.2))
...
comparison_short$ceilingmort<-as.factor(comparison_short$ceilingmort*100)
...
ggsave(path = "result_figs","seasonal_occ_arplot.tif",device="tiff",width=12,height=6,units=c("in"))  
