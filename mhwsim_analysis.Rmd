...
ggplot()+
  ...
  scale_color_manual(name="Species Strategy",
                       labels=c("Acute Tolerator","Chronic Tolerator"),
                      values=c("royalblue","tomato3"))+
  ...
  annotate("text",x=30,y=5,label="16.9 minutes, 39.5 °C ",size=4)+scale_x_continuous(breaks=seq(26,46,by=4))+
...
result.df<-data.frame("temp"=tempi,"p-value"=NA)
...
#Are time to deaths at each CTmax significantly different?
acute<-temp.df%>%filter(species=="highct_highz",temp=="46")
chronic<-temp.df%>%filter(species=="lowct_lowz",temp=="45")

sp.comp<-bind_rows(acute,chronic)

t.test(10^(sp.comp$time)~sp.comp$species)

ggplot(sp.comp,aes(x=species,y=10^(time)))+geom_boxplot()
...
thick_legend<-get_legend(ggplot(category_prop_species,aes(x=hours_dur,y=magnitude,z=cum.mort,group=species))+geom_tile(aes(fill=cum.mort,color=as.factor(category), height=.4, width=18),size=2)+scale_fill_viridis_c()+theme_classic()+geom_contour(color="white")+ geom_text_contour(aes(z = cum.mort),color="white",fontface="bold")+
...
species_heatmap_plot<-ggplot(category_prop_species,aes(x=hours_dur,y=magnitude,z=cum.mort,group=species))+geom_tile(aes(fill=cum.mort,color=as.factor(category)),size=.5)+scale_fill_viridis_c()+theme_classic()+geom_contour(color="white")+ geom_text_contour(aes(z = cum.mort),color="white",fontface="bold")+
  #scale_color_brewer(palette="YlOrRd")+
  scale_color_manual(values=c("cornsilk3","goldenrod2","orangered","firebrick4","purple"))+
  labs(color="MHW Category", fill="Survival (%)")+
  geom_segment(x=5*24,xend=5*24,y=.75,yend=8.25,color="blue",linetype="dashed",linewidth=.75)+
