...
ggplot()+geom_line(data=clim,aes(x=t,y=temp),alpha=0.3)+geom_line(data=clim,aes(x=t,y=thresh),color="yellow")+geom_line(data=clim,aes(x=t,y=seas),color="black")+geom_line(data=clim,aes(x=t,y=thresh_2x),color="orange")+geom_line(data=clim,aes(x=t,y=thresh_3x),color="red")+geom_line(data=clim,aes(x=t,y=thresh_4x),color="purple")
...
species_heatmap_plot<-ggplot(category_prop_species,aes(x=hours_dur,y=magnitude,z=cum.mort,group=species))+geom_tile(aes(fill=cum.mort,color=category),size=0.5)+scale_fill_viridis_c()+theme_classic()+geom_contour(color="white")+ geom_text_contour(aes(z = cum.mort),color="white",fontface="bold")+
  ...
  ggtitle("Species Adaptation Strategies")+ylab("Magnitude (°C)")
...
annual_var_barplot<-result%>%ggplot()+geom_bar(aes(y=total_leveldens,x=category,fill=ceilingmort_f),position='stack',stat='identity')+
  scale_fill_viridis_d(direction=-1)+labs(x="MHW Category",y="Survival Isocline Proportion",fill="Survival Isoclines")+theme_classic()+facet_wrap(.~label)+ggtitle("Annual Variation")+theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
occ_plot<-ggplot(data=allseasons_bound_hour)+
  ...
  scale_color_manual(values=c("Climatology"="black",
                              ...
                              "IV Extreme"='purple',"Simulated MHW"="darkgray"))+
...
occ_plot2<-ggplot(data=allseasons_bound_hour,aes(x=datetime,y=merged_temp))+
  ...
  theme_classic()+
  geom_line(aes(x=datetime,y=thresh),color="yellow",linetype=2,linewidth=1,inherit.aes = F)+
  geom_line(aes(x=datetime,y=thresh_2x),color="orange",linetype=2,linewidth=1,inherit.aes = F)+
  geom_line(aes(x=datetime,y=thresh_3x),color="red",linetype=2,linewidth=1,inherit.aes = F)+
