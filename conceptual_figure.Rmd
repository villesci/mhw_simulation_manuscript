  ...
  for (j in seq_len(ncol(DT)))
    ...
    maxtemp<-out_res%>%filter(datetime>as.POSIXct('2022-01-01 00:00:00'))%>%filter(vals==max(vals))%>%select(vals)
      ...
      scale_color_manual(values=c("Climatology"="darkgray","Observed Temperature"="black",
                                  "90% Threshold"="goldenrod2","2x Threshold"="orangered","3x Threshold"="firebrick4",
                                  "4x Threshold"='purple'),
                        breaks = c("90% Threshold", "2x Threshold", "3x Threshold", "4x Threshold", "Observed Temperature","Climatology"))+
      scale_fill_manual(values=c("I Moderate"=alpha("goldenrod2",1),"II Strong"=alpha("orangered",1),
                                 "III Severe"=alpha("firebrick4",1),"IV Extreme"=alpha('purple',1)))+
...
#only the first hypothetical species is used
...
#  annotate("text",x=30,y=5,label="16.9 minutes, 39.53 °C ",size=4)
 ...
 C_plot<-ggplot()+
      geom_line(data=simulated_line,aes(y=(time),x=temp,color=species),linewidth=1,inherit.aes=F,color="dimgray")+
      theme_minimal()+
      geom_point(data=simulated_point,aes(y=time,x=temp,color=species),size=3,shape=3,color="darkgray")+
      labs(y="Time to death (min)",x="Temperature (°C)",color="Species")+scale_y_continuous(position='right')+
      theme(axis.title.y = element_text(colour = "darkgray"),axis.text.y = element_text(colour = "darkgray"),
            panel.grid.major = element_line(colour = "darkgray", linetype = "dashed",size=.3),
            panel.grid.minor = element_line(colour="darkgray",linetype="dotted",size=.1)
)
    ...
    D<-ggplot()+
      geom_line(data=simulated_line,aes(y=time,x=temp,color=species),linewidth=1,inherit.aes=F,color="red")+
      theme_classic()+scale_y_log10()+
      geom_point(data=simulated_point,aes(y=time,x=temp,color=species),size=3,color="black")+
...
```{r,echo=F}
ggarrange(B,D,MHW_example,nrow=3,ncol=1,labels=c("A","B","C"))
