...
clim_2022<-clim_obj%>%dplyr::filter(t>=as.POSIXct('2022-07-22')&t<as.POSIXct('2022-09-12'))
    ...
    maxtemp<-out_res%>%dplyr::filter(datetime>as.POSIXct('2022-01-01 00:00:00'))%>%dplyr::filter(vals==max(vals))%>%select(vals)
      ...
      geom_segment(aes(x=maxtemptime,xend=maxtemptime-lubridate::hours(2),y=25.6,yend=25.95),arrow=arrow(length=unit(0.2,"cm")))+
      geom_segment(aes(x=end_date_mhw,xend=maxtemptime+lubridate::minutes(30),y=30,yend=28.5),arrow=arrow(length=unit(0.2,"cm")))
...
ggsave("result_figs/conceptual_figure.pdf",width=15,height=5,units="in",dpi=300,device='pdf')   
