...
#official MHW color scale + cornsilk (this paper)
MHW_colours <- c(
  'No MHW'="cornsilk3",
  "I Moderate" = "#ffc866",
  "II Strong" = "#ff6900",
...
maxtemptime<-out_res%>%dplyr::filter(datetime>as.POSIXct('2022-01-01 00:00:00'))%>%dplyr::filter(vals==max(vals))%>%select(datetime)
maxtemptime<-as.POSIXct(maxtemptime[,1])
reps_list<-unique(tri_data$reps)
clim_2022<-clim_obj%>%dplyr::filter(t>=as.POSIXct('2022-07-22')&t<as.POSIXct('2022-09-12'))
    ...
    system.time(joined_df<-as.data.table(tri_data)[as.data.table(out_res),on='datetime'])
    system.time(f_dowle3(joined_df))
    
    joined_df<-as.data.frame(joined_df)
    joined_df$combined_temperature <- joined_df$vals + joined_df$value #value is triangle, vals is 
    joined_df_short<-joined_df%>%
      dplyr::filter(datetime>=start_date_rep)%>%
      dplyr::filter(datetime<end_date_rep)
    ...
    repi_selection<-tri_data%>%select(c(value,datetime))%>%rename("t"=datetime)%>%
      dplyr::filter(t>=start_date_rep&t<end_date_rep)
    ...
    #joined_df_short %>% mutate(group = factor(group, levels = c('1', '0')))
    maxtemp<-out_res%>%dplyr::filter(datetime>as.POSIXct('2022-01-01 00:00:00'))%>%dplyr::filter(vals==max(vals))%>%select(vals)
      ...
      geom_line(aes(x=time,y=thresh,color="90% Threshold"),linetype=2,linewidth=1)+
      geom_line(aes(x=time,y=thresh_2x,color="2x Threshold"),linetype=2,linewidth=1)+
      geom_line(aes(x=time,y=thresh_3x,color="3x Threshold"),linetype=2,linewidth=1)+
      geom_line(aes(x=time,y=thresh_4x,color="4x Threshold"),linetype=2,linewidth=1)+
      ...
      scale_fill_manual(values=c("I Moderate"=alpha(MHW_colours[2][[1]],1),"II Strong"=alpha(MHW_colours[3][[1]],1),
                                 "III Severe"=alpha(MHW_colours[4][[1]],1),"IV Extreme"=alpha(MHW_colours[5][[1]],1)))+
      theme_classic()+labs(x="Time",y="Temp. (°C)",fill="MHW Category",color="MHW Threshold")+
        geom_blank(aes(color="Climatology"))+
      geom_blank(aes(fill="Observed Temperature",value="black"))+
        #guides(fill="none")+
        theme(axis.title.x=element_blank())+
      annotate("segment", x = maxtemptime, xend = maxtemptime, y = 26, yend = 31, linetype = 3)+
      annotate("segment", x = maxtemptime-lubridate::days(5),xend=maxtemptime+lubridate::days(5),y=26,yend=26,linetype=3)+

      scale_y_continuous(limits=c(25.25,31))+
      annotate("text",x=maxtemptime,y=25.4,label="MHW Duration = 10 days",size=4,fontface="italic")+
      annotate("text",x=end_date_mhw+lubridate::days(5),y=30,label="Magnitude =\n 5°C ",size=4,fontface="italic")+
      annotate("segment", x=maxtemptime,xend=maxtemptime-lubridate::hours(2),y=25.6,yend=25.95,arrow=arrow(length=unit(0.2,"cm")))+
      annotate("segment", x=end_date_mhw,xend=maxtemptime+lubridate::minutes(30),y=30,yend=28.5,arrow=arrow(length=unit(0.2,"cm")))
...
#only the first hypothetical species is used - highct_highz
example_TDT<-data.frame("species"=c("highct_highz"),"z"=c(4.152792),"ctmax"=c(48))
...
int_x <- (models$lowct_lowz$coefficients[1]-models$highct_highz$coefficients[1])/
  (models$highct_highz$coefficients[2]-models$lowct_lowz$coefficients[2])      
    ...
    D<-ggplot()+
      geom_line(data=simulated_line,aes(y=time,x=temp,color=species),linewidth=1,inherit.aes=F,color="black")+
      theme_classic()+scale_y_log10()+
      geom_point(data=simulated_point,aes(y=time,x=temp,color=species),size=3,alpha=0.3)+
      scale_color_brewer(palette="Set1")+
      labs(y="Log Time to death (min)",x="Temp. (°C)",color="Species")+
      theme(axis.title.y = element_text(colour = "black"),axis.text.y = element_text(colour = "black"),
            panel.grid.major = element_line(colour = "black", linetype = "dashed",size=0.3),
            panel.grid.minor = element_line(colour="black",linetype="dotted",size=.1))+
...
static2<-static2%>%rename(c("t_coma"="time","assay_temp"="temp"))
    ...
    tmpi<-rezende_min(example_mhw,temp.df,tc=26)
    ...
    exmple_mort<-tmpi%>% dplyr::filter(type=="cum.mort")
    min_mort<-round(min(exmple_mort$mort),digits=1)
    ...
    MHW_example<-ggplot() +
      ...
      scale_y_continuous(name="Survivorship Proportion",limits=mort_limits, sec.axis = sec_axis(~(. - min(mort_limits)) / trans + min(temp_limits), name = "Temp. (°C)"))+
      guides(color="none")+theme_classic()+
        ...
        annotate("text",x=as.POSIXct("2022-08-27"),y=75,label=paste0("Cumulative \n Survivorship = ", min_mort,"% "),size=4,fontface="italic")+
        geom_segment(aes(x=as.POSIXct("2022-08-27"),xend=max(joined_df_short$time)-lubridate::hours(12),y=80,yend=90),arrow=arrow(length=unit(0.2,"cm")))
...
```{r,echo=F}
ggarrange(B,D,MHW_example,nrow=1,ncol=3,labels=c("A","B","C"))
      
ggsave("result_figs/conceptual_figure.pdf",width=15,height=5,units="in",dpi=600,device='pdf') 
ggsave("result_figs/conceptual_figure.svg",width=15,height=5,units="in",dpi=600,device='svg') 

ggsave("result_figs/conceptual_figure.tif",width=15,height=5,units="in",dpi=300,device='tiff')   
